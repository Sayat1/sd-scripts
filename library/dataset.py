    ...
    def random_color_background(self, image: np.ndarray, variant: int = 0):
        def get_random_origin_color():
            if len(self.color_pool) == 0:
                self.color_pool.append(np.array([255, 0, 0], dtype=np.uint8))
                self.color_pool.append(np.array([0, 255, 0], dtype=np.uint8))
                self.color_pool.append(np.array([0, 0, 255], dtype=np.uint8))
                self.color_pool.append(np.array([255, 255, 255], dtype=np.uint8))
                self.color_pool.append(np.array([0, 0, 0], dtype=np.uint8))
        ...
        if image.shape[2] == 4:
            ...
            if (alpha == 1.0).all():
                logger.warning(
                    "random_color_background is applied to an image with alpha channel but all pixels are fully opaque. This should not happen."
                )
                return image
            color = get_random_origin_color()
            background = np.full(image[:, :, :3].shape, color, dtype=np.uint8)
            image_rgb = image[:, :, :3]
            image_rgb = (image_rgb.astype(np.float32) * alpha + background.astype(np.float32) * (1.0 - alpha)).astype(
                np.uint8
            )
        ...
        for i, info in enumerate(tqdm(image_infos)):
            ...
            if caching_strategy.cache_to_disk:
                ...
                cache_available = caching_strategy.is_disk_cached_outputs_expected(
                    te_out_npz, subset.text_encoder_cache_variations
                )
                if cache_available:  # do not add to batch
                    continue
            ...
            for v in range(max_variations):
                for i, info in enumerate(batch):
                    ...
                    if v < subset.text_encoder_cache_variations:
                        info.caption = self.process_caption(subset, original_captions[i])
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.text_encoder_outputs is not None:
                ...
                # on disk
                v_idx = (
                    random.randint(0, subset.text_encoder_cache_variations - 1)
                    if subset.text_encoder_cache_variations > 1
                    else None
                )
