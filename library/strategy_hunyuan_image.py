...
class HunyuanImageLatentsCachingStrategy(LatentsCachingStrategy):
    ...
    def is_disk_cached_latents_expected(self, bucket_reso: Tuple[int, int], npz_path: str, flip_aug: bool, alpha_mask: bool, random_crop_variations: int = 1):
        return self._default_is_disk_cached_latents_expected(32, bucket_reso, npz_path, flip_aug, alpha_mask, multi_resolution=True, random_crop_variations=random_crop_variations)

    def load_latents_from_disk(
        self, npz_path: str, bucket_reso: Tuple[int, int], variation_index: Optional[int] = None
    ) -> Tuple[Optional[np.ndarray], Optional[List[int]], Optional[List[int]], Optional[np.ndarray], Optional[np.ndarray]]:
        return self._default_load_latents_from_disk(32, npz_path, bucket_reso, variation_index=variation_index)  # support multi-resolution

    # TODO remove circular dependency for ImageInfo
    def cache_batch_latents(
        self, vae: hunyuan_image_vae.HunyuanVAE2D, image_infos: List, flip_aug: bool, alpha_mask: bool, random_crop: bool, random_crop_variations: int = 1
    ):
