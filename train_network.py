...
class NetworkTrainer:
    ...
    def generate_step_logs(
        ...
        if args.optimizer_type.lower().startswith("automagic"):
        ...
        for i, lr in enumerate(lrs):
            ...
            if args.optimizer_type.lower().startswith("automagic"):
                if isinstance(opt_lrs, list):
                    opt_lr = opt_lrs[i]
                    if isinstance(opt_lr, list):
                        logs[f"lr/auto_lr/{lr_desc}"] = opt_lr[0]
                        logs[f"lr/auto_lr_min/{lr_desc}"] = opt_lr[2]
                        logs[f"lr/auto_lr_max/{lr_desc}"] = opt_lr[1]
                    else:
    ...
    def process_batch(
        ...
        depth_loss = None
        if args.depth_consistency_weight > 0 and (not use_loss_split or not step_is_diffusion or (args.depth_consistency_preview_every > 0 and global_step % args.depth_consistency_preview_every == 0)):
            # Need x0_pred
            ...
            x0_pixels = depth_utils.decode_latents_to_pixels(vae, x0_pred / self.vae_scale_factor, model_type)
            ...
            if gt_depth_list is not None and depth_consistency_manager is not None:
                ...
                # zero out depth loss if it's a diffusion step and we are splitting
    ...
    def train(self, args):
        ...
        try:
            if support_multiple_lrs:
                results = network.prepare_optimizer_params_with_multiple_te_lrs(text_encoder_lr, args.unet_lr, args.learning_rate, split_te=args.split_te, split_conv=args.split_conv)
            else:
        ...
        if is_modal:
            #progress_bar.dynamic_miniters = True
            progress_bar.file=sys.stderr
            #progress_bar.smoothing=0.05
